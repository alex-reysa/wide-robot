...
| --- | --- | --- |
| **2E-1** | Git/versioned workspace hygiene | 🟡 `.gitignore` excludes Python caches, local venvs, logs, and benchmark outputs. Benchmark, comparison, and invalid-fixture reports include `sourceProvenance` with a deterministic source snapshot and Git commit/status when available. Remaining release step: regenerate final reports after the release commit so provenance is Git-backed and clean. |

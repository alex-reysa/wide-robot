...
| **4** | Cross-source flagship task | 🟡 **Sony/tripod leg landed (Phase 3A); MuJoCo put_cube_in_tray exists (2C); external-sim leg scaffolded (2F-4 offline half landed — RLBench PutItemInDrawer converter/targets/tests green, live capture pending).** Remaining: the 2F-4 live capture and the unified cross-source report (Phase 6) tying object_inside_container across MuJoCo + external sim + Sony/tripod through the unchanged verifier. |
...
RLBench evidence source yet). **Result (E)** opens the external-sim leg of the flagship
`object_inside_container` task with a second RLBench task, `PutItemInDrawer`: the offline
half (two-body converter, two targets, synthetic fixture, 24-test no-RLBench suite) is
landed and green; the 9-demo live capture is the documented follow-on (2F-4).
...
| **2F-4** | External object-inside-container task | 🟡 **Offline half landed.** RLBench `PutItemInDrawer` chosen as the external-sim container source: a two-body converter (`pilots/rlbench/adapter_object_inside_container.py`), two pilot targets (`object_inside_container_{terminal_only,relation_event}.json`), a recorder (`record_put_item_in_drawer.py`), a committed synthetic fixture, and `tests/test_rlbench_object_inside_container.py` (24 tests, no RLBench: synthetic success PASSes both targets non-vacuously, born-inside FAILs relation_event on `initial_state` only, near/rim/far/flat FAIL leakage-clean, leaky/non-neutral rejected at the door, source-identity quarantined, off-task confusion clean) — all green, `csg/` frozen. **Remaining:** the 9-demo live Runpod capture → committed live fixtures → flip on the live tests (external successes PASS, leakage-clean, `physicalValidity: null`). |

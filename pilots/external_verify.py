...
Source-agnostic driver shared by every external pilot (RLBench, real-camera, …). The
benchmark's ``run_one`` does ``solve(target) -> extract_robot_csg -> match ->
leakage_report``; here the solver is replaced by an external trace produced by a pilot
adapter, and the *same* frozen ``extract_robot_csg -> match -> leakage_report`` runs
unchanged. Nothing in ``csg`` is imported in an altered form — the verifier does not know
where the trace came from, which is exactly what makes a clean PASS meaningful.

PASS criterion is identical to ``csg.benchmark.run_one``: the hard-probe matcher passes,
the trace is leakage-clean, and physical validity ``is not False`` (an external kinematic
trace reports ``null`` → *physics-unverified*, an honest PASS label, never *valid*).

Extracted verbatim from ``pilots/rlbench/run_external.py``; the RLBench module now
re-exports these names so existing imports and the ``python -m pilots.rlbench.run_external``
CLI keep working. UNCERTAIN (low-confidence perception) is **not** decided here — a pilot
that needs it (e.g. ``pilots.real_camera.verify_episode``) layers a fail-closed
evidence-quality gate *before* calling :func:`verify_external_rollout`.
"""
...
def verify_external_rollout(
    ...
    *,
    ...
    Raises :class:`pilots.external_rollout.ExternalTraceLeakage` if the rollout
...
def load_gold_targets(gold_dir: str | Path) -> Dict[str, Json]:
    ...
    …) — the same neutral case names ``csg.benchmark`` uses, never source labels.
    """
    root = Path(gold_dir)
    out: Dict[str, Json] = {}
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--rollout", required=True, help="external csg.rollout.v0 json (from a pilot adapter)")
